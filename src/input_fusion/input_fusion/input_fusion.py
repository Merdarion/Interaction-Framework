...
class MergedInputPublisher(Node):
    ...
    def __init__(self):
        ...
        # For the qualisys integration of the experiment -> not necessary without Qualisys MoCap environment
        ...
        # Args:
        #     quaternion (tuple): A tuple or list containing the quaternion (w, x, y, z).
        ...
        # T = np.array([
        #     [1 - 2 * (y**2 + z**2), 2 * (x * y - z * w), 2 * (x * z + y * w), pos_x],
        #     [2 * (x * y + z * w), 1 - 2 * (x**2 + z**2), 2 * (y * z - x * w), pos_y],
        #     [2 * (x * z - y * w), 2 * (y * z + x * w), 1 - 2 * (x**2 + y**2), pos_z]
        # ])
        ...
        #     eyetracker_position = transformation_matrix(self.eyetracker_position, self.eyetracker_rotation)           
            

        #     kinect_position = transformation_matrix(self.kinect_position, self.kinect_rotation)    
        #     kinect_inverse = np.linalg.inv(kinect_position)
    ...
    def merger(self):
        ...
        # Scene description and objects gets initialized
        if self.initialized == False and type(self.rgb_image) == np.ndarray:
            try:
        ...
        # Set on/off object tracking -> not fully implemented yet!
        ...
        if not self.audio_queue.empty():
            ...
            if self.block_transmission:
                ...
                # Detect positions of objects in world coordinates
                if len(self.detected["class_name"]) > 0:
                ...
                # Optionally detect fixated object based on gaze data
                ...
                # Collecting the data for the final dictionary
                if self.detected:
